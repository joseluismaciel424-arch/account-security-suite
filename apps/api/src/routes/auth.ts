...
const createSession = async (userId: string, token: string, req: express.Request, mfaVerified: boolean) => {
  ...
  await pool.query(
    ...
    [userId, hashToken(token), deviceName, userAgent, req.ip ?? null, mfaVerified, new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)],
  );
};
...
authRouter.post('/register', async (req, res) => {
  ...
  try {
    ...
    const user = result.rows[0]; const token = createAccessToken(user.id);
  ...
  } catch { return res.status(409).json({ message: 'Unable to create account.' }); }
});
...
authRouter.post('/login', async (req, res) => {
  ...
  if (user.mfa_locked_until && new Date(user.mfa_locked_until) > new Date()) return res.status(429).json({ message: 'MFA temporarily locked. Try again later.' });
  if (!(await verifyPassword(password, user.password_hash))) {
    await recordSecurityEvent({ userId: user.id, type: 'login-failed', req });
    return res.status(401).json({ message: 'Invalid email or password.' });
  }
  if (user.mfa_enabled) return res.status(202).json({ requiresMfa: true, challengeToken: createMfaChallenge(user.id) });
  const newDevice = await isNewDevice(user.id, req);
  const token = createAccessToken(user.id);
  await createSession(user.id, token, req, false);
  if (newDevice) await recordSecurityEvent({ userId: user.id, type: 'new-device-signin', req, metadata: { deviceName: req.headers['x-device-name'] ?? 'Unknown device' } });
  return res.json({ token, user: { id: user.id, email: user.email, mfaEnabled: false } });
});

authRouter.post('/mfa/verify', async (req, res) => {
  const challengeToken = String(req.body?.challengeToken ?? ''); const code = String(req.body?.code ?? '');
  try {
    ...
    const valid = user.mfa_secret_encrypted && verifyTotpCode(decryptMfaSecret(user.mfa_secret_encrypted), code);
    if (!valid) {
      const attempts = Number(user.mfa_failed_attempts ?? 0) + 1;
      const lockedUntil = attempts >= 5 ? new Date(Date.now() + 15 * 60 * 1000) : null;
      await pool.query('UPDATE users SET mfa_failed_attempts=$1,mfa_locked_until=$2 WHERE id=$3', [attempts, lockedUntil, user.id]);
      await recordSecurityEvent({ userId: user.id, type: attempts >= 5 ? 'mfa-locked' : 'mfa-failed', req, metadata: { attempts } });
      return res.status(attempts >= 5 ? 429 : 401).json({ message: attempts >= 5 ? 'MFA temporarily locked.' : 'Invalid MFA code.' });
    }
    await pool.query('UPDATE users SET mfa_failed_attempts=0,mfa_locked_until=NULL WHERE id=$1', [user.id]);
    const newDevice = await isNewDevice(user.id, req); const token = createAccessToken(user.id);
    await createSession(user.id, token, req, true);
    await recordSecurityEvent({ userId: user.id, type: 'mfa-verified', req });
    if (newDevice) await recordSecurityEvent({ userId: user.id, type: 'new-device-signin', req });
...
authRouter.post('/logout', async (req, res) => {
  const token = req.headers.authorization?.replace('Bearer ', '').trim(); if (!token) return res.status(401).json({ message: 'Authentication required.' });
  try { const payload = verifyToken(token); await pool.query('UPDATE sessions SET revoked_at=NOW() WHERE token_hash=$1 AND user_id=$2', [hashToken(token), payload.sub]); return res.json({ message: 'Session revoked successfully.' }); } catch { return res.status(401).json({ message: 'Invalid or expired token.' }); }
});
