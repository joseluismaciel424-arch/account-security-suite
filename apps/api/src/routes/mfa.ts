...
mfaRouter.post('/enable', async (req, res) => {
  try {
    ...
    await pool.query('UPDATE users SET mfa_enabled=true,recovery_codes_hashes=$1::jsonb,updated_at=NOW() WHERE id=$2', [JSON.stringify(await Promise.all(codes.map(hashRecoveryCode))), req.user!.id]);
...
mfaRouter.post('/recovery/verify', async (req, res) => {
  ...
  const hashes = Array.isArray(result.rows[0]?.recovery_codes_hashes) ? result.rows[0].recovery_codes_hashes as string[] : [];
  for (let index = 0; index < hashes.length; index += 1) {
    if (await verifyRecoveryCode(String(req.body?.code ?? '').trim(), hashes[index])) {
      const remaining = hashes.filter((_hash, itemIndex) => itemIndex !== index);
